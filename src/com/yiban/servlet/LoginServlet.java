	...
	//		System.out.println("用户名："+stuId+"；密码："+pass);
			if(YzmCode == null || yzmText == null){
				...
				if(true == user.isLogin(stuId,pass)){
					...
					out.print("{\"code\":301,\"Msg\":\"登录成功\",\"url\":\""+request.getContextPath()+"/index.jsp\"}");
				}else{
			...
			jdbcBean.addLog(null, null, null, "testlogin",stuId+"尝试登录失败；登录IP："+StringCode.getRealIp(request)+"；UA："+request.getHeader("user-agent"));
			out.print("{\"code\":200,\"Msg\":\"密码错误！\"}");
			e.printStackTrace();
		}catch(Exception e){
